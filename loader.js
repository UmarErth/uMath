...
const games = [
    ...
    { 
        name: "Cookie Clicker", 
        icon: "🧬", 
        content: `<base href="https://jsdelivr.net">
<html>
<head>
<script>
...
window.renderGames = function(list) {
    ...
    grid.innerHTML = list.map((game, index) => {
        const gameIdx = games.findIndex(g => g.name === game.name);
        return `
            <div class="game-card" onclick="launchGame(${gameIdx})">
...
// 4. START SITE
window.onload = () => window.renderGames(games);
